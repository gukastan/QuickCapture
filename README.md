...
- Mac 로그인 시 QuickCapture 자동 시작 설정
...
- 영역을 선택해 파일로 저장 (`⌘ + shift + 4`)
- 영역을 선택해 클립보드에 복사 (`⌘ + shift + control + 4`)
- 창을 선택해 클립보드에 복사 (`⌘ + shift + control + 4 → space`)
- 전체 화면을 클립보드에 복사 (`⌘ + shift + control + 3`)

최초 기본값은 `영역을 선택해 클립보드에 복사`입니다. 캡처 후 ChatGPT, Slack, 문서 등에 `⌘ + V`로 바로 붙여넣을 수 있습니다.
...
- **좌클릭·option + X 실행 동작**: 네 가지 캡처 기능 중 기본 동작 선택
- **Mac 로그인 시 QuickCapture 자동 시작**: 자동 시작 켜기 또는 끄기
- **전역 실행 단축키 사용 (option + X)**: 전역 단축키 켜기 또는 끄기
- **화면 캡처 권한 설정 열기… (시스템 설정)**: macOS 권한 설정 열기
- **화면 캡처 권한 다시 설정… (문제 발생 시)**: 꼬인 권한 기록 초기화
- **QuickCapture 종료 (⌘ + Q)**: 앱 종료
...
2. `QuickCapture.app`을 `control + 클릭`하거나 우클릭합니다.
3. 메뉴에서 **열기**를 선택합니다.
...
우클릭 메뉴의 **화면 캡처 권한 설정 열기… (시스템 설정)**를 사용해 해당 설정으로 바로 이동할 수도 있습니다.
...
4. **화면 캡처 권한 다시 설정… (문제 발생 시)**를 선택합니다.
...
`영역을 선택해 파일로 저장 (⌘ + shift + 4)`은 바탕 화면에 다음과 같은 이름으로 파일을 저장합니다.

```text
QuickCapture_2026-07-27_12-34-56.png
```

클립보드 복사 기능은 캡처 이미지를 시스템 클립보드에 넣으며 `⌘ + V`로 붙여넣을 수 있습니다.
